...
| 10 | 지침이 시스템 업그레이드에서 쿠버네티스 패키지를 뺌 | 설명 | 통과 | — | — | 이 지침은 시스템 업그레이드에서 쿠버네티스 패키지를 빼 둔다. |
| 11 | 뺀 이유: kubeadm·쿠버네티스 업그레이드는 별도 주의 | 설명 | 통과 | — | — | 쿠버네티스 업그레이드는 따로 조심해야 해서 패키지를 빼 둔다. |
| 12 | apt 인덱스 갱신 후 리포지터리용 패키지 설치 | 절차 | 실패 | KSTL-DOC-001 | Style | 한 문장에 갱신과 설치가 있다. 다음 한 동작은 `apt` 인덱스 갱신이다. |
| 13 | apt 색인 갱신, 세 도구 설치, 버전 고정 | 절차 | 실패 | KSTL-DOC-001 | Style | 한 문장에 갱신·설치·고정이 있다. 다음 한 동작은 `apt` 색인 갱신이다. 묻기: 12의 「인덱스」와 13의 「색인」이 같은 apt 목록인가? ① 같다 ② 다르다. 용어표 없음. TER-001은 실패로 치지 않음. |
| 14 | 런타임과 kubelet cgroup 드라이버를 맞출 것, 아니면 오류 | 설명 | 실패 | KSTL-SYN-002 | Accuracy | 묻기: cgroup 드라이버를 맞추는 사람은 누구인가? ① 노드 설치자 ② 클러스터 관리자. |

10·11은 `#### 경고` 상자이나 패키지 hold·업그레이드 주의라 안전 위험이 아니다. KSTL-SAF-001 비적용. 14도 구성 주의라 SAF-001 비적용. 행위자를 지어내지 않는다.
...
| A7 | 예 | SYN-002만 물었다. SAF-001은 패키지 hold·구성 주의에 비적용. |
...
|---|---|---|
| F2 | 절차 | yes — 9, 12, 13 |
| F3 | 경고 | no — `#### 경고` 상자는 있으나 안전 위험 문장 없음. SAF-001 비적용 |
| F4 | 게이트 | yes — SYN-002. TER-001은 불명으로 물음 |

D1·D2는 절차에만. 9는 다음 행동이 하나. 12·13은 원문이 둘 이상이라 실패. 14는 주체를 원문에서 확정하지 못해 물었다.
...
- 패키지 hold·업그레이드 주의·cgroup 구성 주의는 `#### 경고` 상자여도 안전 위험이 아니다. SAF-001 비적용.
- 12는 인덱스, 13은 색인. 용어표가 없어 TER-001은 실패로 단정하지 않고 불명으로 물었다.
- 일반 절차의 생략 주어는 설치 독자 하나로 보고 SYN-002를 걸지 않았다. cgroup 주의만 주체가 갈릴 수 있어 물었다.

통과 8 · 실패 6 · EXP-002 ×1 · EXP-003 ×1 · EXP-005 ×1 · DOC-001 ×2 · SYN-002 ×1 · 불명 TER-001 ×1
